...
async def init_db():
    ...
    async with engine.begin() as conn:
        ...
        try:
            await conn.run_sync(
                lambda c: c.exec_driver_sql(
                    "UPDATE strategies SET wick_min_move_pct=3.0 "
                    "WHERE wick_min_move_pct IS NULL"
                )
            )
        except Exception:
            pass
        ...
        try:
            await conn.run_sync(
                lambda c: c.exec_driver_sql(
                    "UPDATE strategies SET wick_martingale_mode='price_and_wt' "
                    "WHERE wick_martingale_mode IS NULL OR wick_martingale_mode=''"
                )
            )
        except Exception:
            pass
