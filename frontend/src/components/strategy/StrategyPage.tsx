          ...
          <div key={s.id} className="bg-gray-900 border border-gray-800 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-gray-400">
              ...
              {s.signal_source === 'wick_spike' ? (
                <div>接针: <span className="text-gray-200">量×{s.wick_volume_mult ?? 8} ATR×{s.wick_spike_atr_mult ?? 5} 涨跌≥{s.wick_min_move_pct ?? 3}% 回撤≤{s.wick_max_retrace_pct ?? 50}%{(s.wick_amp_vol_relax_enabled ?? true) ? ` 放宽≥${s.wick_vol_relax_progress_start ?? 1}→${s.wick_vol_relax_progress_full ?? 1.5}@${s.wick_vol_relax_mult ?? 5}×` : ''}</span></div>
              ) : null}
